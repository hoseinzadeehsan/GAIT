...
def print_gpu_memory_usage():
    try:
        ...
        print("GPU Memory Usage:", result.decode('utf-8').strip())
    except:
        print("Could not retrieve GPU memory usage. Ensure NVIDIA drivers are installed.")
    ...
    for epoch in range(1,epochs+1):
        ...
        with torch.no_grad():
            for j, (ids, rels, subs, labels) in enumerate(bar2):
                ...
                del ids, rels, subs
                del output, y_pred_prob, y_pred_label
...
if __name__ == '__main__':
    ...
    for round in rounds:
        if True:
            train_loader_path = '../data/tokenized_data/train_'+str(MAX_LEN)+'_fold_'+str(round)+'5fold'+'GNN'
            valid_loader_path = '../data/tokenized_data/valid_'+str(MAX_LEN)+'_fold_'+str(round)+'5fold'+'GNN'
            ...
            for lr in lrs:
                print('start training fold', round, 'learning rate', lr, 'batch size', BS, 'max length', MAX_LEN)
                model = KREL().cuda()
                model_save_path = '../checkpoints/semtab-RECA'+"_lr="+str(lr)+'_bs='+str(BS)+'_max='+str(MAX_LEN)+'_{}_5folds.pkl'.format(round+1)
