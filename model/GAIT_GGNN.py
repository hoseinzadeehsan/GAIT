...
metric_dir = join(configs.root_dir, 'results', 'GGNN')
...
def evaluation(model, graph_batch, loss_fcn):
    model.eval()
    with torch.no_grad():
        edges = torch.zeros(graph_batch.number_of_edges(), dtype=torch.long).to(device)
        logits = model(graph_batch, graph_batch.ndata['feat'], edges)
...
def validate(args, model, valid_dataset, type, save_metric=False):
    model.eval()
    ...
    print("{}:  loss: {:.4f} | accuracy: {:.4f} | macro_f: {:.4f} | weighted_f: {:.4f}"
          .format(type, mean_loss, acc, metric['macro avg']['f1-score'], metric['weighted avg']['f1-score']))

    if save_metric:
        df = pd.DataFrame(metric).transpose()
        df.to_csv(join(metric_dir, 'classification report_{}.csv'.format(type)))
        return
...
def train(args, model, train_dataset, validation_dataset):
    ...
    adr = os.path.join(configs.root_dir, 'saved_models_GGNN',args.model_name + args.data_name + '_{}_{}'.format(args.num_layers, args.num_hidden))
    adr_last = os.path.join(configs.root_dir, 'saved_models_GGNN', args.model_name + args.data_name + 'last_{}_{}'.format(args.num_layers,args.num_hidden))
    adr_state_dict = os.path.join(configs.root_dir, 'saved_models_GGNN',args.model_name + args.data_name + '_state_dict_{}_{}'.format(args.num_layers, args.num_hidden))
    adr_state_dict_best = os.path.join(configs.root_dir, 'saved_models_GGNN',args.model_name + args.data_name + '_state_dict_best_{}_{}'.format(args.num_layers, args.num_hidden))
...
def main(args):
    ...
    model = GGNN(num_feats, args.num_hidden, args.classes, args.num_layers, args.shared_weight, args.dropout)
    ...
    adr_best_model = os.path.join(configs.root_dir, 'saved_models_GGNN',
...
if __name__ == '__main__':
    ...
    parser.add_argument("--classes", type=int, default=78,
                        help="Number of final classes")
    parser.add_argument("--epochs", type=int, default=10,
                        help="number of training epochs")
    parser.add_argument("--num-hidden", type=int, default=[78],
    ...
    parser.add_argument('--data-name', type=str, default='dataset_webtables_4',
    ...
    parser.add_argument('--model-name', type=str, default='tmp',
                        help="saving model")
    parser.add_argument('--shared-weight', default=False, type=bool)
